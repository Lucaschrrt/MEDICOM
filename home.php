...
<body class="home-page">
    ...
    <section id="product"> <!-- La présentation produits est en dur mais le gestionnaire de produit est dynamique -->
        <h2> Présentation de nos produits </h2>
    ...
    <?php
        ...
        class Medicament {
            ...
            public function __construct($nom, $prix, $quantite, $date_ajout, $date_modif, $photo) {
                $this->nom = $nom;
                $this->prix = $prix;
        ...
        $stmt = $pdo->query('SELECT nom, prix, quantite, date_ajout, date_modif, photo FROM medicaments'); // Ajout du champ photo à la requête SQL
        $medicaments = [];
        while ($row = $stmt->fetch())
        {
            $medicaments[] = new Medicament($row['nom'], $row['prix'], $row['quantite'], $row['date_ajout'], $row['date_modif'], $row['photo']); // Ajout du champ photo à la création de l'objet Medicament
        }

        echo "<table>";
        echo "<tr> <th>Nom</th> <th>Prix</th> <th>Quantité</th> <th>Date d'ajout</th> <th>Date de modification</th> <th>Photo</th> </tr>"; // Ajout d'une colonne pour la photo
        ...
        foreach ($medicaments as $medicament) {
            ...
            echo "<td>" . $medicament->date_modif . "</td>";
